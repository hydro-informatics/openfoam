...
a k-ε turbulence model. interFoam identifies the water-air interface 
based on the Volume of Fluid (VOF) method, which solves the transport 
equations for a single or multiple phase fractions α, where α is 0.5 at the 
...
In the **boundary** file, it is necessary to correctly define the type of the
different composing elements. For instance, in the example here below, the 
...
the interFoam solver can be found in the [case folder](). A detailed
overview of all available options can be found in the OpenFOAM User Guide
